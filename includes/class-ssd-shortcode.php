...
class SSD_Shortcode {
    ...
    public function render( $atts, $content = '' ) {
        ...
        $started = SSD_Schema::start( $type, $args );
    ...
    private function parse_list_items( $content, $atts ) {
        ...
        foreach ( $lines as $line ) {
            if ( ! empty( $line ) ) {
                ...
                SSD_Schema::add_item( $line, $pos, ! empty( $atts['url'] ) ? $atts['url'] : null );
            }
        }
    }
    ...
    private function parse_faq_items( $content, $atts ) {
        ...
        foreach ( $lines as $line ) {
            ...
            if ( 2 === count( $parts ) ) {
                SSD_Schema::add_faq( trim( $parts[0] ), trim( $parts[1] ) );
            }
        }
    }
    ...
    private function parse_howto_steps( $content ) {
        ...
        foreach ( $lines as $line ) {
            if ( ! empty( $line ) && '{' !== substr( trim( $line ), 0, 1 ) ) {
                $index++;
                SSD_Schema::add_step( 'Step ' . $index, $line );
            }
        }
    }
